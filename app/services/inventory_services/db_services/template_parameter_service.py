    ...
    async def set_template_parameter_invalid(self, tprm_ids: list[int]) -> None:
        try:
            async with self.uow:
                ...
                template_parameters = (
                    await self.template_parameter_repo.get_template_parameters_by_id(
                ...
                tmo_ids = [templ.template_object_id for templ in template_parameters]
                # Invalidate objects
            ...
            raise Exception(f"Error: {type(ex)}: {ex}")
