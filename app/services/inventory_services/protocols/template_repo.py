...
from domain.template.entities.template import TemplateDTO
...
class TemplateRepo(object):
    ...
    async def set_templates_invalid(
        ...
        result: Sequence[Template] = (await self.session.scalars(statement=stmt)).all()
        if result:
    ...
    async def get_templates_by_tmo_id(
        ...
        stmt = select(Template).where(Template.object_type_id.in_(object_type_ids))
        result: Sequence[Template] = (await self.session.scalars(statement=stmt)).all()
        if result:
            return [TemplateDTO.from_db(template) for template in result]
        return []
