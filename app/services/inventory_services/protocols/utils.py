...
def handle_db_exceptions(
    ...
    async def wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
        ...
        try:
            ...
            logger.error(f"Integrity error in function {func_name}: {ex}.")
            raise ValueError(f"{ex.orig}, {ex.params}, {ex.statement}")
        except ProgrammingError as ex:
            logger.error(f"Programming error in function {func_name}: {ex}.")
            raise ValueError(f"{ex.orig}, {ex.params}, {ex.statement}")
        except TimeoutError:
            ...
            logger.error(f"Unexpected error in function {func_name}: {ex}.")
            raise
